# 📋 PRD - Product Requirements Document

## Maternidade Consciente

**Versão:** 1.0  
**Data:** Janeiro 2026  
**Status:** MVP Completo

---

## 1. Visão Geral

### 1.1 Missão
Empoderar mães com ferramentas digitais para uma maternidade mais organizada, saudável e conectada.

### 1.2 Visão
Ser a plataforma de referência para gestantes e mães no Brasil, oferecendo acompanhamento completo desde a gravidez até os primeiros anos do bebê.

### 1.3 Público-Alvo
- **Primário:** Gestantes e mães de bebês (0-2 anos)
- **Secundário:** Parceiros/pais, familiares que apoiam
- **Terciário:** Profissionais de saúde (pediatras, obstetras)

---

## 2. Objetivos do Produto

### 2.1 Objetivos de Negócio
| Objetivo | Métrica | Meta Q1/2026 |
|----------|---------|--------------|
| Usuários ativos | MAU | 5.000 |
| Retenção D7 | % usuários que retornam | 40% |
| Conversão Premium | % free → paid | 5% |
| NPS | Net Promoter Score | 50+ |

### 2.2 Objetivos de Produto
- ✅ Oferecer ferramentas completas para gestão da maternidade
- ✅ Reduzir ansiedade através de organização e informação
- ✅ Criar comunidade de suporte entre mães
- ✅ Disponibilizar versão offline (PWA)
- 🔄 Integrar com profissionais de saúde

---

## 3. Módulos do Produto

### 3.1 Enxoval do Bebê
**Objetivo:** Ajudar mães a planejar e organizar as compras do enxoval

| Feature | Prioridade | Status |
|---------|------------|--------|
| Lista de itens personalizável | P0 | ✅ Completo |
| Controle de orçamento | P0 | ✅ Completo |
| Compartilhamento com família | P1 | ✅ Completo |
| Guia de quantidades RN | P1 | ✅ Completo |
| Exportação Excel/PDF | P1 | ✅ Completo |
| Alerta de troca (data limite) | P2 | ✅ Completo |

### 3.2 Amamentação
**Objetivo:** Facilitar o registro e acompanhamento da amamentação

| Feature | Prioridade | Status |
|---------|------------|--------|
| Registro de mamadas | P0 | ✅ Completo |
| Timer de amamentação | P0 | ✅ Completo |
| Dashboard com estatísticas | P1 | ✅ Completo |
| Gestão de ordenha | P1 | ✅ Completo |
| Exportação PDF | P2 | ✅ Completo |

### 3.3 Sono do Bebê
**Objetivo:** Monitorar padrões de sono e identificar melhorias

| Feature | Prioridade | Status |
|---------|------------|--------|
| Registro de sono | P0 | ✅ Completo |
| Recomendações por idade | P1 | ✅ Completo |
| Dashboard analítico | P1 | ✅ Completo |
| Comparação com marcos | P2 | ✅ Completo |
| Exportação PDF | P2 | ✅ Completo |

### 3.4 Cartão de Vacinação
**Objetivo:** Manter registro completo e lembretes de vacinas

| Feature | Prioridade | Status |
|---------|------------|--------|
| Calendário SUS/Privado | P0 | ✅ Completo |
| Registro de doses | P0 | ✅ Completo |
| Multi-bebês | P1 | ✅ Completo |
| Lembretes automáticos | P2 | ✅ Completo |
| Exportação PDF | P2 | ✅ Completo |

### 3.5 Monitor de Desenvolvimento
**Objetivo:** Acompanhar marcos de desenvolvimento do bebê

| Feature | Prioridade | Status |
|---------|------------|--------|
| Lista de marcos por idade | P0 | ✅ Completo |
| Registro de conquistas | P0 | ✅ Completo |
| Alertas de atenção | P1 | ✅ Completo |
| Relatório para pediatra | P1 | ✅ Completo |
| Timeline visual | P2 | ✅ Completo |

### 3.6 Guia de Alimentação
**Objetivo:** Orientar nutrição durante gestação e amamentação

| Feature | Prioridade | Status |
|---------|------------|--------|
| Planos semanais | P0 | ✅ Completo |
| Receitas categorizadas | P0 | ✅ Completo |
| Alertas de alimentos | P1 | ✅ Completo |
| IA Nutricional (chat) | P1 | ✅ Completo |
| Geração com IA | P2 | ✅ Completo |
| Lista de compras | P2 | ✅ Completo |

### 3.7 Recuperação Pós-Parto
**Objetivo:** Monitorar recuperação física e emocional

| Feature | Prioridade | Status |
|---------|------------|--------|
| Rastreador de sintomas | P0 | ✅ Completo |
| Controle de medicamentos | P1 | ✅ Completo |
| Diário de autoestima | P1 | ✅ Completo |
| Conquistas de recuperação | P2 | ✅ Completo |
| Acesso para parceiro | P2 | ✅ Completo |

### 3.8 Comunidade
**Objetivo:** Conectar mães para compartilhamento e suporte

| Feature | Prioridade | Status |
|---------|------------|--------|
| Posts e comentários | P0 | ✅ Completo |
| Categorias e tags | P1 | ✅ Completo |
| Sistema de likes | P1 | ✅ Completo |
| Denúncias e moderação | P1 | ✅ Completo |
| Desafios comunitários | P2 | ✅ Completo |

### 3.9 Mala da Maternidade
**Objetivo:** Organizar itens para o hospital

| Feature | Prioridade | Status |
|---------|------------|--------|
| Checklist por categoria | P0 | ✅ Completo |
| Filtro por tipo de parto | P1 | ✅ Completo |
| Compartilhamento | P1 | ✅ Completo |
| Exportação PDF | P2 | ✅ Completo |

### 3.10 Materiais e Ferramentas
**Objetivo:** Oferecer calculadoras e guias úteis

| Feature | Prioridade | Status |
|---------|------------|--------|
| Calculadora de fraldas | P1 | ✅ Completo |
| Comparador de marcas | P2 | ✅ Completo |
| Simulador fralda de pano | P2 | ✅ Completo |
| Sugestões de ferramentas | P2 | ✅ Completo |

---

## 4. Requisitos Não-Funcionais

### 4.1 Performance
| Requisito | Meta | Status |
|-----------|------|--------|
| First Contentful Paint | < 1.5s | ✅ |
| Time to Interactive | < 3s | ✅ |
| Bundle size inicial | < 500KB | ✅ |
| Lighthouse Performance | > 90 | ✅ |
| Code Splitting | Lazy loading | ✅ |
| Virtualização | Listas grandes | ✅ |

### 4.2 Segurança
| Requisito | Status |
|-----------|--------|
| RLS em todas as tabelas | ✅ |
| Validação com Zod | ✅ |
| Rate limiting | ✅ |
| CORS configurado | ✅ |
| Leaked password protection | ✅ |
| Error monitoring (Sentry) | ✅ |

### 4.3 Acessibilidade
| Requisito | Status |
|-----------|--------|
| ARIA labels | ✅ |
| Skip links | ✅ |
| Contraste adequado | ✅ |
| Navegação por teclado | ✅ |
| Focus management | ✅ |

### 4.4 PWA
| Requisito | Status |
|-----------|--------|
| Instalável | ✅ |
| Offline-first | ✅ |
| Background sync | ✅ |
| Push notifications | 🔄 |

### 4.5 Testes
| Requisito | Status |
|-----------|--------|
| Testes unitários (Vitest) | ✅ |
| Testes E2E (Playwright) | ✅ |
| Cobertura core utils | ✅ |
| Cobertura UI components | ✅ |

---

## 5. Backlog Futuro

### 5.1 Prioridade Alta (Q2/2026)
- [ ] Push notifications nativas
- [ ] Integração com calendário do dispositivo
- [ ] Modo multi-idiomas (i18n)
- [ ] App nativo (React Native)

### 5.2 Prioridade Média (Q3/2026)
- [ ] Integração com wearables (sono)
- [ ] Relatórios automáticos para pediatra
- [ ] Gamificação expandida
- [ ] Parcerias com marcas (marketplace)

### 5.3 Prioridade Baixa (Q4/2026)
- [ ] IA de previsão de marcos
- [ ] Telemedicina integrada
- [ ] Comparativo entre bebês
- [ ] API pública para integrações

---

## 6. Métricas de Sucesso

### 6.1 Métricas de Engajamento
- **DAU/MAU:** > 30%
- **Sessões por usuário:** > 3/semana
- **Tempo médio de sessão:** > 5 minutos

### 6.2 Métricas de Feature
- **Itens de enxoval por usuário:** > 30
- **Registros de mamada por dia:** > 5
- **Posts na comunidade por mês:** > 1.000

### 6.3 Métricas de Satisfação
- **NPS:** > 50
- **Rating na App Store:** > 4.5
- **Taxa de churn:** < 5% mensal

---

## 7. Documentos Relacionados

- [README.md](README.md) - Visão geral do projeto
- [ARCHITECTURE.md](ARCHITECTURE.md) - Arquitetura técnica
- [ROADMAP.md](ROADMAP.md) - Fases e timeline
- [PENDENCIAS.md](PENDENCIAS.md) - Status de pendências
- [CONTRIBUTING.md](CONTRIBUTING.md) - Guia de contribuição

---

*Documento atualizado em: Janeiro 2026*
