  ...
  supabase: {
    from: vi.fn(table => {
      ...
      if (table === 'development_milestone_types') {
        ...
        mockReturn.single = vi
          .fn()
          .mockResolvedValue({ data: mockMilestoneRecords[0], error: null });
      }
...
describe('Milestone Age Range', () => {
  ...
  it('should identify upcoming milestones', () => {
    const babyAgeMonths = 2;
    const upcomingMilestones = mockMilestoneTypes.filter(m => m.age_min_months > babyAgeMonths);
...
describe('Milestone Areas', () => {
  ...
  it('should group milestones by area', () => {
    const byArea = mockMilestoneTypes.reduce(
      (acc, m) => {
...
describe('Pediatrician Alerts', () => {
  it('should identify milestones with alerts', () => {
    const milestonesWithAlerts = mockMilestoneTypes.filter(m => m.pediatrician_alert !== null);
...
describe('Progress Statistics', () => {
  ...
  it('should count achievements by area', () => {
    const achievedByArea = mockMilestoneRecords.reduce(
      (acc, r) => {
