      ...
      try {
        ...
        if (itemsData) {
          ...
          const processedItems = itemsData.map((item: any) => ({
            ...
            savings:
              item.qtd_planejada * item.preco_planejado -
              item.qtd_comprada * (item.preco_unit_pago + item.frete - item.desconto),
            savingsPercent: 0,
            store: item.loja,
            link: item.link,
            status: item.status,
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8 space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <EnxovalTable items={items} onEdit={() => {}} onDelete={() => {}} config={config} />
          </CardContent>
        </Card>
      </main>
    </div>
