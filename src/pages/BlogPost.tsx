  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      const { data, error } = await supabase
        .from('blog_posts')
        .select(
          'id, title, slug, content, excerpt, meta_title, meta_description, featured_image_url, og_image_url, author_name, categories, tags, seo_keywords, faq_schema, internal_links, reading_time_min, views_count, published_at, created_at, updated_at, status'
        )
        .eq('slug', slug)
        .eq('status', 'published')
        .single();
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 border-b border-border/50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-14 sm:h-16 items-center justify-between">
          <Link
            to="/"
            className="text-lg sm:text-xl font-display font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent"
          >
            Mãe Consciente
          </Link>
      ...
      {/* Breadcrumbs */}
      <nav
        className="container max-w-4xl pt-6 text-sm text-muted-foreground"
      ...
      <article className="container max-w-4xl py-6 sm:py-10">
          ...
          <div>
            ...
            <div className="my-10 p-6 rounded-xl bg-gradient-to-r from-primary/5 to-primary/10 border border-primary/20 text-center">
              ...
              <Button asChild className="shadow-glow">
                <Link to="/auth">
                  Experimentar Grátis <ArrowRight className="h-4 w-4 ml-1" />
                </Link>
              </Button>
            </div>
              ...
              <section className="my-10">
                ...
                <div className="space-y-4">
                  {post.faq_schema.map((faq, i) => (
                    <details
                      key={i}
                      className="rounded-lg border border-border/50 bg-card p-4 group"
                    >
        ...
        <div className="mt-12 p-8 rounded-2xl bg-gradient-to-r from-primary/10 via-primary/5 to-transparent border border-primary/20 text-center">
          ...
          <p className="text-muted-foreground mb-5 max-w-lg mx-auto">
            Ferramentas, guias e acompanhamento para cada fase da maternidade. Experimente grátis
            por 7 dias.
          </p>
