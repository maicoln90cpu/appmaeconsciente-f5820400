  ...
  useEffect(() => {
    const fetchPosts = async () => {
      const { data } = await supabase
        .from("blog_posts")
        .select("id, title, slug, excerpt, featured_image_url, categories, tags, reading_time_min, views_count, published_at, author_name")
        .eq("status", "published")
      ...
      <div className="min-h-screen bg-background">
        <div className="container py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[1,2,3].map(i => <Skeleton key={i} className="h-[340px] rounded-xl" />)}
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-50 border-b border-border/50 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
        <div className="container flex h-14 sm:h-16 items-center justify-between">
          <Link to="/" className="text-lg sm:text-2xl font-display font-bold bg-gradient-to-r from-primary to-primary/60 bg-clip-text text-transparent">
            Mãe Consciente
          </Link>
          <div className="flex gap-2">
            ...
            <Button asChild size="sm" className="shadow-glow">
              <Link to="/auth">Começar Agora <ArrowRight className="h-4 w-4 ml-1" /></Link>
            </Button>
          </div>
        </div>
      </header>
      ...
      <div className="container py-8 sm:py-12">
        ...
        <div className="mb-8 sm:mb-12">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">
            Dicas, guias e informações para cada fase da maternidade — da gestação ao primeiro ano do bebê.
          </p>
        </div>
          ...
          <>
                    ...
                    <div className="p-6 sm:p-8 flex flex-col justify-center">
                      ...
                      <div className="flex items-center gap-4 text-sm text-muted-foreground">
                        <span className="flex items-center gap-1">
                          ...
                          {format(new Date(heroPost.published_at), "d 'de' MMM, yyyy", { locale: ptBR })}
                        </span>
                        <span className="flex items-center gap-1">
            ...
            {hasMore && (
              <div className="text-center mt-10">
                <Button variant="outline" size="lg" onClick={() => setVisibleCount(v => v + POSTS_PER_PAGE)}>
