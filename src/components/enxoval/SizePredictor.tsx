...
export const SizePredictor = ({
  ...
  const clothingPredictions = useMemo(() => 
    predictClothingSizes(prediction),
    [prediction]
  );

  const diaperPredictions = useMemo(() => 
    predictDiaperSizes(prediction),
    [prediction]
  );
  ...
  const nextSizeChange = weightKg ? getNextSizeChange(
    weightKg, 
    ageMonths, 
    gender, 
    percentile
  ...
  const renderPredictionCard = (pred: SizePrediction, type: "clothing" | "diaper") => {
    const isCurrentSize = type === "clothing" 
      ? pred.size === currentClothingSize 
      : pred.size === currentDiaperSize;
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
          ...
          <Alert className="border-primary/30 bg-primary/5">
            ...
            <AlertDescription>
              <div className="flex flex-wrap items-center gap-2">
                <span>Com <strong>{weightKg}kg</strong>, seu bebê está no tamanho</span>
                ...
                {nextSizeChange && (
                  <span className="text-muted-foreground">
                    Próximo tamanho ({nextSizeChange.nextSize}) em ~{nextSizeChange.estimatedMonthsUntilChange.toFixed(1)} meses.
                  </span>
                )}
              </div>
            </AlertDescription>
              ...
              <Card className="bg-muted/30">
                <CardContent className="p-4">
                  ...
                  <div className="mt-3 pt-3 border-t flex justify-between font-medium">
                    ...
                    <span>
                      ~{diaperPredictions.reduce((sum, p) => sum + (p.diapersNeeded || 0), 0)} fraldas
                    </span>
                  </div>
                </CardContent>
              </Card>
        ...
        <Alert className="bg-muted/30">
          ...
          <AlertDescription>
            <strong>Dica:</strong> Bebês crescem em ritmos diferentes. Use a curva de crescimento 
            do seu bebê (informada pelo pediatra) para previsões mais precisas. Compre menos 
            peças em tamanhos menores (RN, P) pois são usados por pouco tempo.
          </AlertDescription>
        </Alert>
      </CardContent>
    </Card>
