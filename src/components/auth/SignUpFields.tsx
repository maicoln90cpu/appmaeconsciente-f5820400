...
export const SignUpFields = ({
  ...
  <>
    ...
    <div className="space-y-2">
      ...
      <PhoneInput id="whatsapp" value={whatsapp} onChange={setWhatsapp} disabled={loading} error={errors.whatsapp} />
    ...
    <div className="space-y-2">
      ...
      {confirmPassword && (
        <div className={cn("flex items-center gap-1 text-xs", passwordsMatch ? "text-green-600" : "text-destructive")}>
          {passwordsMatch ? (<><Check className="h-3 w-3" />Senhas conferem</>) : passwordsDontMatch ? (<><X className="h-3 w-3" />Senhas não conferem</>) : null}
        </div>
      )}
    </div>
    ...
    <div className="flex items-start space-x-2 p-3 bg-muted/50 rounded-lg border border-border/50">
      <Checkbox id="consent" checked={consentAccepted} onCheckedChange={(checked) => setConsentAccepted(checked as boolean)} disabled={loading} className="mt-0.5" />
      <Label htmlFor="consent" className="text-sm leading-relaxed cursor-pointer text-muted-foreground">
        Eu li e concordo com os{" "}
        <a href="/termos" target="_blank" className="text-primary hover:underline font-medium" onClick={(e) => e.stopPropagation()}>Termos de Uso</a>{" "}
        e a{" "}
        <a href="/privacidade" target="_blank" className="text-primary hover:underline font-medium" onClick={(e) => e.stopPropagation()}>Política de Privacidade</a>
