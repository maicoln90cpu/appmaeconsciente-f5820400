...
export const BlogGenerationLogs = () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Extra stats */}
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-lg">Histórico de Gerações</CardTitle></CardHeader>
        <CardContent>
              ...
              <TableBody>
                {sortedData.length === 0 ? (
                  ...
                  <TableRow key={log.id}>
                    ...
                    <TableCell className="text-xs text-muted-foreground">{format(new Date(log.created_at), "dd/MM HH:mm", { locale: ptBR })}</TableCell>
                    <TableCell className="text-xs font-mono">{log.model_used?.split("/").pop() || "—"}</TableCell>
                    <TableCell className="text-xs">{log.total_tokens?.toLocaleString() || "—"}</TableCell>
                    <TableCell className="text-xs">{log.total_cost_usd ? `$${log.total_cost_usd.toFixed(4)}` : "—"}</TableCell>
