...
export const BlogSettingsPanel = () => {
  ...
  return (
    <div className="space-y-4">
      {/* AI Config */}
      <Card>
        <CardHeader className="pb-3"><CardTitle className="text-lg">Configuração de IA</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select value={form.image_model} onValueChange={(v) => setForm(prev => ({ ...prev, image_model: v }))}>
                <SelectTrigger><SelectValue /></SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input value={form.default_author} onChange={(e) => setForm(prev => ({ ...prev, default_author: e.target.value }))} />
            </div>
          ...
          <div>
            ...
            <Textarea value={form.system_prompt} onChange={(e) => setForm(prev => ({ ...prev, system_prompt: e.target.value }))} rows={6} className="font-mono text-xs" />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex gap-2">
            <Input value={newTopic} onChange={(e) => setNewTopic(e.target.value)} placeholder="Novo tópico..." onKeyDown={(e) => e.key === "Enter" && addTopic()} />
            <Button size="sm" onClick={addTopic} disabled={!newTopic.trim()}><Plus className="w-4 h-4" /></Button>
          </div>
          <div className="flex flex-wrap gap-1.5 max-h-48 overflow-y-auto">
            {form.topics_pool.map((t) => (
              <Badge key={t} variant="secondary" className="gap-1 text-xs">
                {t}
                <button onClick={() => removeTopic(t)} className="ml-1 hover:text-destructive"><X className="w-3 h-3" /></button>
              </Badge>
            ))}
          </div>
        </CardContent>
