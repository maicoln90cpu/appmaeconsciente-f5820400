...
function scoreBadge(score: number) {
  if (score >= 80)
    return (
      <Badge className="bg-emerald-500/20 text-emerald-400 border-emerald-500/30">Saudável</Badge>
    );
  if (score >= 50)
      ...
      {integrations.map(int => {
        ...
        return (
          <Card key={int.name} className="p-4 text-center">
            ...
            <Badge variant="outline" className={`mt-2 ${st.className}`}>
              {st.label}
            </Badge>
          </Card>
        );
        ...
        {aggregated.map((c, i) => (
          <TableRow key={i}>
            ...
            <TableCell>
              <Badge
                variant="outline"
                className={c.lastStatus === 'success' ? 'text-emerald-400' : 'text-red-400'}
              >
                {c.lastStatus}
              </Badge>
            </TableCell>
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <Button
          variant="outline"
          size="sm"
          onClick={() => data && exportAuditLogCSV(data)}
