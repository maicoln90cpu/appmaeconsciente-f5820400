...
import {
  Shield, DollarSign, Send, AlertTriangle, Zap, Gauge, CheckCircle2,
    ...
    <div className="space-y-1">
      <div className="flex justify-between text-sm">
        ...
        <span className={isGood ? "text-emerald-400" : "text-red-400"}>
          {value}{unit} / meta: {invert ? "≤" : "≥"}{target}{unit}
        </span>
      </div>
      <Progress value={capped} className={`h-2 ${isGood ? "[&>div]:bg-emerald-500" : "[&>div]:bg-red-500"}`} />
    </div>
  );
}
...
// ═══════════════════════════════════════════════
function SLAPanel() {
  const { data, isLoading } = useQuery({ queryKey: ["sla-metrics"], queryFn: fetchSLAMetrics, staleTime: 60000 });
...
function AICostPanel() {
  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? <Skeleton className="h-48 w-full" /> : d.daily.length === 0 ? (
...
function DeliveryPanel() {
  const { data, isLoading } = useQuery({ queryKey: ["delivery-status"], queryFn: fetchDeliveryStatus, staleTime: 60000 });
...
function RecentErrorsPanel() {
  const { data, isLoading } = useQuery({ queryKey: ["recent-errors-24h"], queryFn: fetchRecentErrors, staleTime: 60000 });
    ...
    <div className="space-y-4">
      {d.hasSpike && (
        <div className="flex items-center gap-2 p-3 rounded-md bg-destructive/10 border border-destructive/30">
          ...
          <span className="text-sm text-destructive font-medium">⚠️ Pico de erros detectado! Mais de 10 erros/hora nas últimas 24h.</span>
        </div>
      )}
...
function PerformanceMetricsPanel() {
  const { data, isLoading } = useQuery({ queryKey: ["perf-metrics"], queryFn: fetchPerformanceMetrics, staleTime: 60000 });
...
function WebVitalsPanel() {
  const vitals = [
    { name: "LCP", description: "Largest Contentful Paint", good: 2500, adequate: 4000, unit: "ms" },
  ...
  return (
    <div className="space-y-4">
      <p className="text-xs text-muted-foreground">
        Os Web Vitals são coletados no navegador dos usuários. Os thresholds abaixo são referência do Google.
      </p>
          ...
          <Card key={v.name} className="p-4">
            ...
            <div className="space-y-1 text-xs">
              <div className="flex items-center gap-2">
                ...
                <span>Bom: {"<"}{v.good}{v.unit}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-yellow-500" />
                <span>Adequado: {v.good}-{v.adequate}{v.unit}</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-red-500" />
                <span>Ruim: {">"}{v.adequate}{v.unit}</span>
              </div>
            </div>
          </Card>
        ))}
      ...
      <p className="text-xs text-muted-foreground text-center">
        Coleta automatizada de métricas reais será habilitada na etapa de instrumentação do frontend.
      </p>
    </div>
  );
}
      ...
      {isLoading ? <Skeleton className="h-32 w-full" /> : (
        <div className="space-y-2">
          {channels.map((ch, i) => {
            ...
            return (
              <div key={ch.name} className="flex items-center justify-between p-3 rounded-md border">
                <div className="flex items-center gap-2">
