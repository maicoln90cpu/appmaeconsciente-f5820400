...
export const AIEngagementPanel = () => {
  ...
  const [repliesPerRun, setRepliesPerRun] = useState([3]);
  const [likesPerRun, setLikesPerRun] = useState([4]);
  ...
  const runEngagement = useMutation({
    mutationFn: async () => {
      ...
      const { data, error } = await supabase.functions.invoke("auto-engage-community", {
        body: {
          maxPosts: postsPerRun[0],
          maxReplies: repliesPerRun[0],
  ...
  const actionConfig: Record<string, { icon: typeof MessageSquare; label: string; bg: string; text: string }> = {
    post: { icon: FileText, label: "Post", bg: "bg-purple-100 text-purple-600", text: "text-purple-600" },
    comment: { icon: MessageSquare, label: "Comentário", bg: "bg-blue-100 text-blue-600", text: "text-blue-600" },
    like: { icon: Heart, label: "Curtida", bg: "bg-pink-100 text-pink-600", text: "text-pink-600" },
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Sparkles className="h-5 w-5" />Executar Manualmente</CardTitle>
          <CardDescription>Execute agora ou ajuste os parâmetros do cron automático</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              <Label>Posts por execução: {postsPerRun[0]}</Label>
              <Slider value={postsPerRun} onValueChange={setPostsPerRun} min={1} max={10} step={1} />
            </div>
            <div className="space-y-2">
              <Label>Respostas por execução: {repliesPerRun[0]}</Label>
              <Slider value={repliesPerRun} onValueChange={setRepliesPerRun} min={1} max={10} step={1} />
            </div>
            <div className="space-y-2">
              <Label>Curtidas por execução: {likesPerRun[0]}</Label>
              <Slider value={likesPerRun} onValueChange={setLikesPerRun} min={1} max={20} step={1} />
            </div>
          </div>
        ...
        <CardContent>
          {logsLoading ? (
            ...
            <div className="space-y-3 max-h-96 overflow-y-auto">
              {logs.map((log) => {
                const config = actionConfig[log.action_type] || actionConfig.comment;
