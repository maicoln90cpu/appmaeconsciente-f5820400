...
export const UserManagement = () => {
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data: activities, error: actError } = await supabase.rpc('get_user_last_activities', {
        user_ids: userIds,
      });

      if (actError) console.error('Erro ao buscar atividades:', actError);

      const activityMap = new Map((activities || []).map((a: any) => [a.user_id, a]));
  ...
  const { data: productAccess } = useQuery<Record<string, ProductAccessData[]>>({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('user_product_access').select(`
          user_id,
          product_id,
          expires_at,
          products (
  ...
  const grantAccessMutation = useMutation({
    mutationFn: async ({
      ...
      const { error } = await supabase
        ...
        .upsert(
          { user_id: userId, product_id: productId, expires_at: expiresAt },
  ...
  const filteredAndSortedUsers = useMemo(() => {
    ...
    const filtered = users.filter(user => {
      ...
      const isAdmin =
        Array.isArray(user.user_roles) && user.user_roles.some(r => r?.role === 'admin');
  ...
  return (
    <div className="space-y-4">
      ...
      {filteredAndSortedUsers.length === 0 && users && users.length > 0 && (
        <NoSearchResults
          onClearFilters={() =>
            setFilters(prev => ({ ...prev, searchTerm: '', roleFilter: 'all' }))
          }
        />
      )}
      ...
      <div className="grid gap-4">
        {filteredAndSortedUsers.map(user => (
          <UserCard
            ...
            onSelectProduct={(userId, productId) =>
              setAccessState(prev => ({
                ...prev,
                selectedUser: userId,
            ...
            onAccessDurationChange={duration =>
              setAccessState(prev => ({ ...prev, accessDuration: duration }))
            }
            onLifetimeChange={lifetime =>
              setAccessState(prev => ({ ...prev, lifetimeAccess: lifetime }))
            }
