...
import {
  Shield, AlertTriangle, CheckCircle2, Eye, EyeOff, RefreshCw,
  Scan, Clock
} from "lucide-react";
...
export const AutoModerationPanel = () => {
  ...
  const scanAllPosts = useMutation({
    ...
    onSuccess: (data) => {
      toast.success(`Scan completo: ${data.scanned} posts, ${data.flagged} sinalizados, ${data.hidden} ocultos`);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { icon: CheckCircle2, value: stats?.approved || 0, label: "Aprovados", bg: "bg-green-100", color: "text-green-600" },
          { icon: AlertTriangle, value: stats?.flagged || 0, label: "Sinalizados", bg: "bg-yellow-100", color: "text-yellow-600" },
          { icon: EyeOff, value: stats?.hidden || 0, label: "Ocultos", bg: "bg-red-100", color: "text-red-600" },
        ].map((s, i) => (
          <Card key={i}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Scan className="h-5 w-5" />Scanner de Moderação</CardTitle>
          <CardDescription>Analisa os últimos 20 posts aprovados usando IA</CardDescription>
        </CardHeader>
        <CardContent>
          <Button onClick={() => scanAllPosts.mutate()} disabled={isScanning} className="w-full gap-2" size="lg">
            {isScanning ? (
              <><RefreshCw className="h-4 w-4 animate-spin" />Analisando posts...</>
            ) : (
              <><Scan className="h-4 w-4" />Executar Scan de Moderação</>
            )}
          </Button>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            {flaggedPosts.map((post) => (
              <div key={post.id} className="p-4 rounded-lg border bg-yellow-50/50 dark:bg-yellow-900/10 space-y-2">
                ...
                <p className="text-sm">{post.content?.substring(0, 200)}{(post.content?.length || 0) > 200 ? "..." : ""}</p>
                <div className="flex gap-2">
                  <Button size="sm" variant="outline" className="gap-1" onClick={() => reviewPost.mutate({ postId: post.id, action: "approve" })}>
                    <CheckCircle2 className="h-3 w-3" /> Aprovar
                  </Button>
                  <Button size="sm" variant="destructive" className="gap-1" onClick={() => reviewPost.mutate({ postId: post.id, action: "hide" })}>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><Clock className="h-5 w-5" />Histórico de Moderação</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
