    ...
    const load = async () => {
      ...
      feedings?.forEach(f =>
        mapped.push({
          ...
          type: 'feeding',
          description:
            f.feeding_type === 'breast' ? 'Amamentação registrada' : 'Mamadeira registrada',
          time: f.created_at,
        })
      );
      sleeps?.forEach(s =>
      ...
      setActivities(
        mapped.sort((a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()).slice(0, 5)
      );
    };
    load();
  }, [user]);
          ...
          <div className="space-y-2">
            {activities.map(a => (
              <div key={a.id} className="flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${a.type === 'feeding' ? 'bg-pink-500' : 'bg-indigo-500'}`}
                />
