        ...
        <div className="flex items-center justify-around py-1 px-1">
          {mobileNavItems.map((item, index) => {
            ...
            return (
              ...
              >
                <div
                  className={`relative p-1 xs:p-1.5 rounded-xl transition-all duration-200 ${
                    active ? 'bg-primary/10' : ''
                  }`}
                >
                  <Icon
                    className={`h-4 w-4 xs:h-5 xs:w-5 transition-transform duration-200 ${active ? 'scale-110' : ''}`}
                  />
                  {active && (
                ...
                <span
                  className={`truncate max-w-full transition-all duration-200 ${active ? 'font-semibold' : ''}`}
                >
                  {item.label}
                </span>
              </Link>
