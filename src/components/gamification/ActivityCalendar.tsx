...
export const ActivityCalendar = memo(({ showLegend = true }: ActivityCalendarProps) => {
  ...
  const weeks = useMemo(() => {
    ...
    const activityMap = new Map(activityCalendar.map(a => [a.date, a]));
  ...
  const stats = useMemo(() => {
    ...
    let currentStreak = 0;
    const sortedDays = [...activityCalendar].sort(
    ...
    <Card className="overflow-hidden min-w-0">
      ...
      <CardContent className="p-3 sm:p-4">
        ...
        <div className="flex flex-col xs:flex-row gap-2 xs:gap-3 mb-3 sm:mb-4 text-xs sm:text-sm">
          ...
          <div className="text-muted-foreground hidden sm:block">{stats.totalXP} XP</div>
        </div>
              ...
              {weeks.map((week, weekIdx) => (
                <div key={weekIdx} className="flex flex-col gap-0.5">
                  ...
                  <div className="h-2.5 sm:h-3 text-[8px] sm:text-[10px] text-muted-foreground">
                    {weekIdx === 0 ||
                    format(week.weekStart, 'MM') !==
                      format(weeks[weekIdx - 1]?.weekStart || week.weekStart, 'MM')
