...
const BadgeCard = memo(({ badge, isUnlocked, unlockedAt, onClick }: BadgeCardProps) => {
  ...
  return (
    ...
    >
      ...
      <span
        className={`text-xs font-medium text-center leading-tight ${!isUnlocked && 'text-muted-foreground'}`}
      >
        {badge.name}
      </span>
...
const BadgeDetailDialog = memo(
  ({ badge, isUnlocked, unlockedAt, open, onOpenChange }: BadgeDetailDialogProps) => {
