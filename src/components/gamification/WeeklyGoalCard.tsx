      ...
      <CardContent className="space-y-3 sm:space-y-4 px-3 sm:px-6 pb-3 sm:pb-6">
        ...
        <div className="space-y-1.5 sm:space-y-2">
          <div className="flex justify-between text-xs sm:text-sm">
            ...
            <span className="font-medium text-primary">+{totalXPThisWeek} XP</span>
          </div>
        ...
        <div
          className={cn(
            ...
            goalCompleted ? (rewardClaimed ? 'bg-muted' : 'bg-primary/10') : 'bg-muted/50'
          )}
        >
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-medium truncate">
                {rewardClaimed ? 'Resgatada!' : `+${WEEKLY_GOAL_REWARD_XP} XP`}
              </p>
              {!goalCompleted && (
          ...
          {goalCompleted && !rewardClaimed && (
            ...
            >
              {isClaiming ? <Loader2 className="h-3 w-3 animate-spin" /> : 'Resgatar'}
            </Button>
          )}
          {rewardClaimed && (
