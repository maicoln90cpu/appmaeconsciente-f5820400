...
export const RelatorioPediatraDialog = ({
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.text(`${babyProfile.baby_name} - ${summary.age_months} meses`, pageWidth / 2, y, { align: "center" });
      ...
      doc.setFont("helvetica", "normal");
      doc.setFontSize(10);
      doc.text(`Data de nascimento: ${format(new Date(babyProfile.birth_date), "dd/MM/yyyy")}`, 20, y);
      y += 5;
      ...
      // Summary by Area
      const areas = ['motor_grosso', 'motor_fino', 'linguagem', 'cognitivo', 'social_emocional'] as const;
      ...
      if (attentionMilestones.length > 0) {
        ...
        attentionMilestones.forEach(record => {
          ...
          doc.text(`• ${record.milestone?.title} (esperado até ${record.milestone?.age_max_months} meses)`, 25, y);
          y += 5;
        });
        ...
        doc.setFontSize(8);
        doc.setTextColor(100);
        doc.text("Nota: Estes marcos passaram da idade típica sem registro. Vale comentar na consulta.", 25, y);
        doc.setTextColor(0);
      }
      ...
      // Save
      doc.save(`relatorio-desenvolvimento-${babyProfile.baby_name.toLowerCase().replace(/\s/g, '-')}.pdf`);
    } catch (error) {
    ...
    <Dialog>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Gere um relatório profissional em PDF com todos os marcos de desenvolvimento
            registrados para levar à consulta.
          </DialogDescription>
        </DialogHeader>
