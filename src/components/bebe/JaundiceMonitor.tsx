...
export const JaundiceMonitor = ({ babyProfileId }: JaundiceMonitorProps) => {
  const { logs, isLoading, addLog, deleteLog, hasDangerSigns, latestLog } =
    useJaundiceLogs(babyProfileId);
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground mt-1">
                  ...
                  {latestLog?.alert_signs &&
                    latestLog.alert_signs.length > 0 &&
                    ` com sinais: ${latestLog.alert_signs.join(', ')}`}
          ...
          <CardContent className="space-y-4 border-t pt-4">
            ...
            <div>
              <Label className="text-sm font-medium mb-2 block">
                Selecione a zona amarelada mais distante
              </Label>
            ...
            <div>
              <Label className="text-sm font-medium mb-2 block">
                Sinais de alerta (marque se presente)
              </Label>
        ...
        <CardContent className="pt-4">
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm text-muted-foreground space-y-1">
              ...
              <p>
                Pressione suavemente a pele do bebê em local com boa luz natural. Observe onde a cor
                amarela aparece — quanto mais distante do rosto, maior o nível de bilirrubina.
              </p>
              <p className="text-xs">
                ⚠️ Esta ferramenta <strong>não substitui</strong> avaliação médica. Em caso de
                dúvida, procure o pediatra.
              </p>
            </div>
          </div>
        </CardContent>
            ...
            <div className="space-y-2">
              {logs.map(log => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${KRAMER_ZONES[log.kramer_zone - 1].color}`}
                    >
                      {log.kramer_zone}
                    </div>
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        ...
                        {log.alert_signs &&
                          log.alert_signs.length > 0 &&
                          ` • ${log.alert_signs.length} alerta(s)`}
                      </p>
                    </div>
                  </div>
                  <Button
