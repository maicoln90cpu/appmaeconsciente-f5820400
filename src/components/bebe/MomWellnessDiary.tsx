...
export const MomWellnessDiary = () => {
  ...
  const chartData = [...logs].reverse().slice(-14).map(log => ({
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            {todayLog ? "Você já registrou hoje — atualize se quiser" : "Registre rapidamente como se sente"}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5">
          ...
          <div className="space-y-2">
            ...
            <Slider value={[anxiety]} onValueChange={([v]) => setAnxiety(v)} min={1} max={5} step={1} />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Slider value={[sleepHours]} onValueChange={([v]) => setSleepHours(v)} min={0} max={14} step={0.5} />
          </div>
          ...
          <Button onClick={handleSave} disabled={upsertLog.isPending} className="w-full">
            {upsertLog.isPending ? "Salvando..." : todayLog ? "Atualizar registro" : "Salvar registro"}
          </Button>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
              ...
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-2xl">{ENERGY_LABELS[Math.round(weeklyAvg.energy)].split(" ")[0]}</p>
              ...
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-2xl">{weeklyAvg.pain > 5 ? "😣" : weeklyAvg.pain > 2 ? "😕" : "😊"}</p>
                <p className="text-xs text-muted-foreground mt-1">Dor</p>
                <p className="text-sm font-medium">{weeklyAvg.pain}/10</p>
              </div>
              <div className="text-center p-3 bg-muted/50 rounded-lg">
                <p className="text-2xl">{ANXIETY_LABELS[Math.round(weeklyAvg.anxiety)].split(" ")[0]}</p>
            ...
            <div className="h-48">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Tooltip />
                  <Line type="monotone" dataKey="Humor" stroke="hsl(var(--primary))" strokeWidth={2} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="Energia" stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} />
                  <Line type="monotone" dataKey="Ansiedade" stroke="#ef4444" strokeWidth={2} dot={{ r: 3 }} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </CardContent>
