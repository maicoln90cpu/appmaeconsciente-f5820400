  ...
  const timelineEvents = useMemo(() => {
    ...
    feedingLogs?.slice(0, 20).forEach((log) => {
      ...
      events.push({
        ...
        description: log.volume_ml ? `${log.volume_ml}ml` : log.duration_minutes ? `${log.duration_minutes}min` : undefined,
    ...
    milestones?.filter(m => m.status === 'achieved').slice(0, 10).forEach((m) => {
      events.push({
        id: `milestone-${m.id}`,
        type: "milestone",
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Visualize todos os eventos e marcos do bebê
        </CardDescription>
      </CardHeader>
      <CardContent>
        {groupedEvents.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">
            ...
            <p className="text-sm mt-1">
              Os registros de sono, mamadas e marcos aparecerão aqui
            </p>
          </div>
        ) : (
