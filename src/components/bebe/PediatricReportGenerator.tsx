  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      if (sections.basicInfo) {
        ...
        yPos += 5;
        doc.text(
          `Data de Nascimento: ${format(new Date(babyProfile.birth_date), 'dd/MM/yyyy', { locale: ptBR })}`,
          14,
          yPos
        );
        yPos += 5;
        ...
        if (babyProfile.birth_type) {
          doc.text(
            `Tipo de Parto: ${babyProfile.birth_type === 'normal' ? 'Normal' : 'Cesárea'}`,
            14,
            yPos
          );
          yPos += 5;
      ...
      if (sections.appointments) {
        ...
        if (upcomingAppointments && upcomingAppointments.length > 0) {
          const aptData = upcomingAppointments
            .slice(0, 5)
            .map(a => [
              a.title,
      ...
      // Save
      doc.save(
        `relatorio-pediatrico-${babyProfile.baby_name.replace(/\s+/g, '-').toLowerCase()}-${format(new Date(), 'yyyy-MM-dd')}.pdf`
      );
    } catch (error) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Gere um PDF completo para levar nas consultas médicas</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        {babyProfile && (
          <div className="p-4 bg-muted/50 rounded-lg">
            ...
            <div className="space-y-1 text-sm text-muted-foreground">
              <p>
                👶 {babyProfile.baby_name} - {calculateAge(babyProfile.birth_date)}
              </p>
        ...
        {/* Generate Button */}
        <Button onClick={generatePDF} disabled={isGenerating} className="w-full gap-2" size="lg">
          {isGenerating ? (
            <>
