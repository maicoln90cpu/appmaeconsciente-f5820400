...
export const CalendarioVacinas = ({ profile, calendar, vaccinations, onRegisterClick }: CalendarioVacinasProps) => {
  ...
  const getVaccineStatus = (vaccine: VaccinationCalendar) => {
    const applied = vaccinations.find(v => 
      v.vaccine_name === vaccine.vaccine_name && 
      v.dose_label === vaccine.dose_label
    );
  ...
  return (
    <div className="space-y-6">
      {Object.entries(groupedVaccines).sort(([a], [b]) => Number(a) - Number(b)).map(([ageMonths, vaccines]) => (
        <Card key={ageMonths}>
          <CardHeader>
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      {applied && (
                        <div className="text-sm text-muted-foreground">
                          Aplicada em: {new Date(applied.application_date).toLocaleDateString('pt-BR')}
                        </div>
                      )}
                    </div>
                        ...
                        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                          ...
                          <div className="space-y-4">
                            ...
                            {vaccine.side_effects && (
                              <div>
                                <h4 className="font-semibold mb-1">⚠️ Efeitos colaterais esperados</h4>
