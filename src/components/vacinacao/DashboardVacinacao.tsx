...
export const DashboardVacinacao = ({ profile, calendar, vaccinations }: DashboardVacinacaoProps) => {
  ...
  const stats = useMemo(() => {
    ...
    const upcomingVaccines = calendar.filter(v => v.age_months > babyAgeMonths && v.age_months <= babyAgeMonths + 3);

    // Próxima vacina pendente (atrasada ou futura)
    const nextVaccine = overdueVaccines.length > 0
      ? overdueVaccines[0]
  ...
  return (
    <div className="space-y-4">
      ...
      {stats.nextVaccine && (
        <Card className={`border-2 ${stats.nextVaccineIsOverdue ? "border-red-400 dark:border-red-600 bg-red-50 dark:bg-red-950/30" : "border-primary/40 bg-primary/5"}`}>
          <CardContent className="py-5">
            <div className="flex items-start gap-4">
              <div className={`p-3 rounded-full shrink-0 ${stats.nextVaccineIsOverdue ? "bg-red-100 dark:bg-red-900" : "bg-primary/10"}`}>
                <Syringe className={`h-6 w-6 ${stats.nextVaccineIsOverdue ? "text-red-600 dark:text-red-400" : "text-primary"}`} />
              </div>
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-3 mt-1 text-sm text-muted-foreground">
                  {stats.nextVaccine.dose_label && (
                    <span>{stats.nextVaccine.dose_label}</span>
                  )}
