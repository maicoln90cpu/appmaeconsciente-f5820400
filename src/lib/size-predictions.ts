...
export const predictClothingSizes = (
  prediction: BabyGrowthPrediction
): SizePrediction[] => {
  ...
  CLOTHING_SIZES.forEach((sizeInfo, index) => {
    ...
    const startMonth = index === 0 ? 0 : findMonthForWeight(
      sizeInfo.minWeight, 
      gender, 
      percentile,
      0
    ...
    const endMonth = findMonthForWeight(
      sizeInfo.maxWeight, 
      gender, 
      percentile,
      startMonth
...
export const predictDiaperSizes = (
  prediction: BabyGrowthPrediction
): SizePrediction[] => {
  const results: SizePrediction[] = [];
  const { gender, percentile, currentAge } = prediction;
