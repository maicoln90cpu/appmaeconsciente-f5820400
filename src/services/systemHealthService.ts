...
export async function fetchAuditLog(days: number = 30) {
  const { data, error } = await supabase
    ...
    .select("id, admin_id, action, entity_type, entity_id, old_values, new_values, metadata, created_at")
    .order("created_at", { ascending: false })
    .limit(200);
  if (error) throw error;
  return data ?? [];
...
export async function fetchCronJobLogs(days: number = 30) {
  ...
  const { data, error } = await supabase
    .from("cron_job_logs")
    .select("id, job_name, schedule, status, duration_ms, error_message, executed_at, metadata, created_at")
    .gte("created_at", since)
...
export function groupErrorsByComponent(errors: Awaited<ReturnType<typeof fetchClientErrors>>) {
  const map = new Map<string, { component: string; message: string; count: number; lastOccurrence: string }>();
...
export function aggregatePerformance(logs: Awaited<ReturnType<typeof fetchPerformanceLogs>>) {
  ...
  for (const l of logs) {
    ...
    if (existing) {
      ...
      map.set(key, { operation: l.operation_name, type: l.operation_type, durations: [l.duration_ms] });
    }
  }
...
export function aggregateCronJobs(logs: Awaited<ReturnType<typeof fetchCronJobLogs>>) {
  const map = new Map<string, { job: string; schedule: string; runs: number; successes: number; totalDuration: number; lastRun: string; lastStatus: string }>();
  for (const l of logs) {
