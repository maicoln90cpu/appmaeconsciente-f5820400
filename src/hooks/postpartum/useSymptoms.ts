  ...
  const { data: symptoms, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, user_id, date, bleeding_intensity, pain_level, fever, healing_status, breast_pain, energy_level, sleep_quality, appetite, bowel_movement, urination, swelling, temperature, cramps_level, notes, created_at, updated_at'
        )
        .eq('user_id', userId)
...
function checkAlerts(symptom: PostpartumSymptom) {
  ...
  if (symptom.bleeding_intensity === 'very_heavy') {
    toast.error('⚠️ ALERTA', {
      description: 'Sangramento muito intenso. Procure atendimento médico imediatamente!',
    });
  } else if (symptom.bleeding_intensity === 'heavy') {
    toast.error('⚠️ Atenção', {
      description:
        'Sangramento intenso detectado. Monitore de perto e contate seu médico se persistir.',
    });
  }
  ...
  // Energia muito baixa
  if (
    symptom.energy_level !== undefined &&
    symptom.energy_level !== null &&
    symptom.energy_level <= 1 &&
    symptom.sleep_quality &&
    symptom.sleep_quality < 4
  ) {
    toast('💙 Cuide-se', {
