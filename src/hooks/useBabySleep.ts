...
export const useBabySleep = () => {
  ...
  const loadSettingsAndMilestones = useCallback(async () => {
    try {
      ...
      const [settingsResponse, milestonesResponse] = await Promise.all([
        supabase.from('baby_sleep_settings').select('*').eq('user_id', user.id).maybeSingle(),
        supabase.from('baby_sleep_milestones').select('*').order('age_range_start'),
      ]);
  ...
  const addSleepLog = useCallback(
    async (logData: Omit<BabySleepLog, 'id' | 'user_id' | 'created_at' | 'updated_at'>) => {
  ...
  const updateSleepLog = useCallback(
    async (id: string, updates: Partial<BabySleepLog>) => {
