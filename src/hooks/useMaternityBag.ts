...
const DEFAULT_ITEMS: Record<string, string[]> = {
  ...
  Bebê: ['Body manga curta', 'Body manga longa', 'Mijão ou calça', 'Macacão', 'Meia', 'Toalha'],
  Acompanhante: [
...
export const useMaternityBag = () => {
  ...
  const categoriesQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, user_id, name, icon, delivery_type_filter, display_order, created_at, updated_at'
        )
        .eq('user_id', user.id)
        .order('display_order');
  ...
  const itemsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select(
          'id, user_id, category_id, name, quantity, checked, notes, cesarean_only, normal_only, created_at, updated_at'
        )
        .eq('user_id', user.id);
  ...
  const updateItemMutation = useMutation({
    mutationFn: async ({
      ...
      itemId: string;
      updates: Partial<MaternityBagItem>;
    }) => {
      const { data, error } = await supabase
        .from('maternity_bag_items')
        .update(updates)
    ...
    onMutate: async ({ itemId, updates }) => {
      ...
      const previousItems = queryClient.getQueryData<MaternityBagItem[]>(
  ...
  const deleteItemMutation = useMutation({
    mutationFn: async (itemId: string) => {
      const { error } = await supabase.from('maternity_bag_items').delete().eq('id', itemId);
